 ...
 */
public final class Jrxml6To7Converter
{
    // ---------------- CLI / options ----------------
...
private static void convertAllElements(Element root, Document doc, Log log)
    ...
    private static void convertSubreportContents(Element legacy, Element dst, Document doc)
	{
		...
		for (int i = 0; i < kids.getLength(); i++)
		{
			...
			// Standardize 'subreportVariable' -> 'fromVariable' (legacy -> v7)
			if (rv.hasAttribute("subreportVariable") && !rv.hasAttribute("fromVariable"))
			{
				rv.setAttribute("fromVariable", rv.getAttribute("subreportVariable"));
				rv.removeAttribute("subreportVariable");
			}
