 ...
 */
public final class Jrxml6To7Converter_PATCHED
{
    // ---------------- CLI / options ----------------
    ...
    private static ConversionResult convertSingle(Path inFile, Path outFile, Opts opts, CorpusModel corpus, Log log) throws Exception
    {
        ...
        // Drop legacy <reportFont> declarations (obsolete in modern JRXML; not supported by our JR7 corpus).
        removeLegacyReportFont(root, log);
        ...
        // In JR 7 element-kind JRXML, most top-level sections are "band-like" and must not contain a <band> child.
        // Flatten <section><band ...>...</band></section> into <section height="..." ...>...</section>.
        flattenSingleBandReportSections(root, doc, log);
    ...
    private static void removeLegacyReportFont(Element root, Log log)
    {
        ...
        while (child != null)
        {
            ...
            if (child.getNodeType() == Node.ELEMENT_NODE && "reportFont".equals(((Element) child).getTagName()))
            {
        ...
        if (removed > 0)
        {
            log.info("Dropped legacy <reportFont> declaration(s): " + removed);
        }
    }
...
// ---------------- XML parse / write ----------------
    ...
    /**
     * JasperReports 7 "element-kind" JRXML represents most *top-level* sections as a single-band container:
     *
     ...
     *   </columnHeader>
     *
     * In legacy JRXML (DTD/XSD) these same sections were written as:
     *
     *   <columnHeader>
     *     <band height="15" isSplitAllowed="true"> ... </band>
     *   </columnHeader>
     *
     * If we leave the legacy <band> wrapper under the section, JR 7's Jackson-based loader will fail
     * because it does not expect a "band" property there.
     *
     * NOTE: This flattening is ONLY for sections that are *direct children of <jasperReport>*.
     * Group headers/footers and <detail> still legitimately contain <band> elements in JR 7.
     */
    private static void flattenSingleBandReportSections(Element jasperReportRoot, Document doc, Log log)
    {
        // Sections that are single-band at the top level in element-kind JRXML.
        ...
        {
            Node child = jasperReportRoot.getFirstChild();
            while (child != null)
            {
                Node next = child.getNextSibling();
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE)
                {
                    ...
                    if (sectionName.equals(section.getTagName()))
                    {
                        flattenSingleBandSection(section, doc, log);
                    }
                }
    ...
    private static void flattenSingleBandSection(Element section, Document doc, Log log)
    {
        // Find element children (ignore whitespace text and comments).
        Element band = null;
        ...
        // Only flatten the common legacy structure: exactly one element child which is <band>.
        if (elementChildCount == 1 && band != null)
        {
            // Copy band attributes to the section (height, splitType, etc.).
            copyAttributesNormalized(band, section);

            // Some JR 7 top-level sections do not accept band-only attributes like splitType.
            // We copy band attributes for height/layout, then strip known-illegal ones per section.
            if (section.hasAttribute("splitType"))
            {
                String tag = section.getTagName();
                if ("columnHeader".equals(tag) || "columnFooter".equals(tag)
                        || "lastPageFooter".equals(tag) || "noData".equals(tag))
                {
                    section.removeAttribute("splitType");
                }
            }


            // Move all children from <band> into the section (in place of the band).
            Node insertBefore = band;
            while (band.getFirstChild() != null)
            {
                Node n = band.getFirstChild();
                band.removeChild(n);
                section.insertBefore(n, insertBefore);
            }

            section.removeChild(band);

            log.info("Flattened <" + section.getTagName() + "><band .../></" + section.getTagName() + "> into band-like section");
        }
        else if (band != null)
        {
            // Legacy JRXML can theoretically contain unexpected structures; we flag it.
            log.warn("Section <" + section.getTagName() + "> contains <band> but is not the single-child legacy form; leaving as-is");
        }
    }
...
private static void convertAllElements(Element root, Document doc, Log log)
	...
	{
		// Subreport in element-kind style supports children such as:
		//   <parametersMapExpression>, <connectionExpression> / <dataSourceExpression>, <expression>,
		//   <returnValue>, and <parameter>.
		//
		// We have an observed JasperReports 7.x deserialization failure for subreport parameters when a
		// dataSourceExpression is present (JRDesignSubreportParameter being treated as a String).
		// Workaround: when legacy subreport has explicit subreportParameter entries AND uses dataSourceExpression,
		// fold those parameters into a single parametersMapExpression and omit <parameter> children.

		Element legacyParametersMapExpression = firstChildElementByTag(legacy, "parametersMapExpression");
		Element legacyConnectionExpression    = firstChildElementByTag(legacy, "connectionExpression");
		Element legacyDataSourceExpression    = firstChildElementByTag(legacy, "dataSourceExpression");
		Element legacySubreportExpression     = firstChildElementByTag(legacy, "subreportExpression");

		// Collect direct child <subreportParameter> elements in original order.
		List<Element> legacySubreportParams = new ArrayList<>();
		NodeList kids = legacy.getChildNodes();
		for (int i = 0; i < kids.getLength(); i++)
		{
			Node n = kids.item(i);
			if (n instanceof Element e && "subreportParameter".equals(e.getTagName()))
			{
				legacySubreportParams.add(e);
			}
		}

		final boolean useParametersMapWorkaround = legacyDataSourceExpression != null && !legacySubreportParams.isEmpty();

		// 1) parametersMapExpression (either legacy or synthesized)
		if (useParametersMapWorkaround)
		{
			String baseMapExpr = (legacyParametersMapExpression != null) ? extractTrimmedText(legacyParametersMapExpression) : null;

			StringBuilder mapExpr = new StringBuilder();
			if (baseMapExpr != null && !baseMapExpr.isBlank())
			{
				// Clone the base map (null-safe), then add the legacy subreportParameter entries.
				mapExpr.append("new java.util.HashMap(")
				      .append("(").append(baseMapExpr).append(") == null ? java.util.Collections.emptyMap() : (").append(baseMapExpr).append(")")
				      .append(")");
			}
			else
			{
			...
			// Use an instance initializer so we can build the map in a single Java expression (allows null values).
			mapExpr.append(" {{");
			for (Element oldParam : legacySubreportParams)
			{
				String pName = oldParam.getAttribute("name");
				Element oldExpr = firstChildElementByTag(oldParam, "subreportParameterExpression");
				String pExpr = (oldExpr != null) ? extractTrimmedText(oldExpr) : null;
				if (pExpr == null || pExpr.isBlank())
				{
					pExpr = "null";
				}

				String escName = (pName == null) ? "" : pName.replace("\\", "\\\\").replace("\"", "\\\"");
				mapExpr.append(" put(\"").append(escName).append("\", ").append(pExpr).append(");");
			}
			mapExpr.append(" }}");

			Element pme = doc.createElement("parametersMapExpression");
			pme.appendChild(doc.createCDATASection(mapExpr.toString()));
			dst.appendChild(pme);
		}
		else if (legacyParametersMapExpression != null)
		{
			Element copied = (Element) doc.importNode(legacyParametersMapExpression, true);
			normalizeExpressionContainer(copied, doc);
			dst.appendChild(copied);
		}

		// 2) connectionExpression / dataSourceExpression (order matches known JR7 examples)
		if (legacyConnectionExpression != null)
		{
			Element copied = (Element) doc.importNode(legacyConnectionExpression, true);
			normalizeExpressionContainer(copied, doc);
			dst.appendChild(copied);
		}
		if (legacyDataSourceExpression != null)
		{
			Element copied = (Element) doc.importNode(legacyDataSourceExpression, true);
			normalizeExpressionContainer(copied, doc);
			dst.appendChild(copied);
		}

		// 3) subreport design expression
		if (legacySubreportExpression != null)
		{
			String exprText = extractTrimmedText(legacySubreportExpression);
		...
		// 4) returnValue (direct children, preserve order)
		for (int i = 0; i < kids.getLength(); i++)
		{
			Node n = kids.item(i);
			...
			if (!"returnValue".equals(e.getTagName()))
			{
				continue;
			}

			Element rv = doc.createElement("returnValue");
			copyAttributesNormalized(e, rv);

			// Standardize legacy v6 attribute to JR 7 naming: fromVariable -> subreportVariable
			// (JR7 returnValue expects 'subreportVariable', not 'fromVariable'.)
			if (rv.hasAttribute("fromVariable") && !rv.hasAttribute("subreportVariable"))
			{
				rv.setAttribute("subreportVariable", rv.getAttribute("fromVariable"));
				rv.removeAttribute("fromVariable");
			}
		...
		// If we used parametersMapExpression workaround, we OMIT <parameter> children entirely.
		if (!useParametersMapWorkaround)
		{
			for (Element oldParam : legacySubreportParams)
			{
				...
				Element oldExpr = firstChildElementByTag(oldParam, "subreportParameterExpression");
				String valExpr = (oldExpr != null) ? extractTrimmedText(oldExpr) : null;

				if (valExpr != null && !valExpr.isBlank())
				{
					Element expr = doc.createElement("expression");
					expr.appendChild(doc.createCDATASection(valExpr));
					newParam.appendChild(expr);
				}

				dst.appendChild(newParam);
			}
		}
	}
    ...
    private static void moveConvertedChildren(Element legacy, Element dst)
    {
        ...
        for (Node k : kids)
        {
            ...
            dst.appendChild(ke.cloneNode(true));
        }
    }
    ...
    private static final class JrxmlValidator
    {
        ...
        private void validateElement(Element e, String path, ValidationReport r)
        {
            ...
            // Many JRXML constructs allow printWhenExpression directly under band containers;
            // corpus may miss these due to being non-path-aware.
            if (BAND_CONTAINERS.contains(tag))
            {
                allowedChildren.add("printWhenExpression");
            }
